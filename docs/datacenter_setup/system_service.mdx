...
The recommended way to authenticate a node is with an API key, which you can create in the CloudRift Console. Set the `CLOUDRIFT_API_KEY` environment variable for the rift service:
...
You can also configure credentials interactively using the rift command line tool:
